...
impl MountedMap {
    ...
    pub fn add_value(&mut self, path : &str, value : &str, data : RegValue) {
        let (hkey, rest) = match path.split_once(|v| v == '/' || v == '\\') {
            Some(v) => v,
            None => return self.cell.entry(path.to_string()).or_insert(MountedCell::new(path)).add_value("", value, data)
        };
        self.cell.entry(hkey.to_string()).or_insert(MountedCell::new(hkey)).add_value(rest, value, data);
    }
...
impl MountedCell {
    ...
    pub fn add_value(&mut self, path : &str, value : &str, data : RegValue) {
        ...
        let (first, rest) = match path.split_once(|v| v == '/' || v == '\\') {
            Some(v) => v,
            None => {
                self.keys.entry(path.to_string()).or_insert(MountedCell::new(path)).add_value("", value, data);
                return;
            }
        };
        self.keys.entry(first.to_string()).or_insert(MountedCell::new(first)).add_value(rest, value, data);
    }
    pub fn get_value(&self, path : &str, value : &str) -> Option<RegValue> {
        if path.is_empty() {
            return self.values.get(value).and_then(|v| Some(v.clone()))
        }
        let (first, rest) = match path.split_once(|v| v == '/' || v == '\\') {
            Some(v) => v,
